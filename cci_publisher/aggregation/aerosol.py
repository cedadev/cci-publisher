...
# Local imports
from .base import CCIAggregationCreator
...
class CCIAerosolDatasetReader(NetcdfDatasetReader):
    """
    Customisation on the basic NetCDF Dataset Reader for
    CCI Aerosol Data to deal with different time formats
    and time attr names
    """
    ...
    def get_datetime(cls, date_str):
        ...
        :param date_str: Date string to try to parse
        :type date_str: str
    ...
    def get_start_end_date(self):
        ...
        if (hasattr(self.ds, "title") and "GOMOS" in self.ds.title and
            ...
            start = epoch + timedelta(
                days=int(self.ds.startDate)
            )

            end = epoch + timedelta(
                days=int(self.ds.endDate),
                hours=23,
                minutes=59,
                seconds=59
            )
        ...
        raise ValueError(f"Could not determine start and end time for file "
                         "'{self.ds.filepath()}'")
    ...
    def get_coord_values(self, dimension):
        ...
        # Get midpoint in days
        midpoint = (start.timestamp() + end.timestamp()) / 2
        midpoint = [int(midpoint / (60 * 60 * 24))]

        return UNITS, midpoint


class CCIAerosolAggregationCreator(CCIAggregationCreator):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.dimension != "time":
            raise ValueError(f"Aerosol special case only handles time "
                             "aggregations - not '{self.dimension}'")
